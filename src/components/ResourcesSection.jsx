      ...
      >
        ...
        <div
          className={`absolute inset-0 border rounded-2xl p-6 flex flex-col ${
            dark ? 'bg-white/[0.03] border-white/[0.07]' : 'bg-white border-gray-100 shadow-sm'
          }`}
        ...
        >
          ...
          <p className={`text-sm leading-relaxed flex-1 ${dark ? 'text-white/50' : 'text-gray-500'}`}>{tool.why}</p>
          <div className={`mt-4 text-xs font-medium ${dark ? 'text-white/25' : 'text-gray-400'}`}>Tap to see how to use →</div>
        </div>
        ...
        >
          ...
          <p className={`text-white/75 text-sm leading-relaxed flex-1 ${dark ? 'text-white/75' : 'text-gray-700'}`}>{tool.how}</p>
          <div className={`mt-4 text-xs font-medium ${dark ? 'text-white/30' : 'text-gray-400'}`}>Tap to flip back ←</div>
        </div>
      </motion.div>
    </motion.div>
  );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            {inspoSites.map((s, i) => (
              ...
              >
                ...
                <div className={`text-xs leading-relaxed ${dark ? 'text-white/40' : 'text-gray-400'}`}>{s.desc}</div>
              </motion.a>
            ))}
          </div>
        </div>
      ...
      <section id="resources" className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          >
            ...
            <p className={`text-sm max-w-lg mx-auto ${dark ? 'text-white/40' : 'text-gray-500'}`}>Chosen for relevance and usefulness, not just popularity. Mixes English and Hindi so you can learn in the language that feels easiest.</p>
          </motion.div>

          {/* Tab toggle */}
          <div className="flex justify-center mb-8">
            <div className={`inline-flex border rounded-full p-1 ${
            dark ? 'bg-white/[0.04] border-white/[0.07]' : 'bg-gray-100 border-gray-200'
          }`}>
              ...
              <motion.a
                ...
                className={`flex items-start gap-3 p-4 border rounded-xl transition-all group ${
                  dark
                    ...
                    : 'bg-white border-gray-100 shadow-sm hover:border-red-300'
                }`}
              >
                ...
                <div>
                  <div className={`text-sm font-medium leading-snug transition-colors ${dark ? 'text-white/80 group-hover:text-white' : 'text-gray-700 group-hover:text-gray-900'}`}>{v.title}</div>
                  <div className={`text-xs mt-1 ${dark ? 'text-white/30' : 'text-gray-400'}`}>{v.channel}</div>
                </div>
              </motion.a>
            ))}
          </div>
